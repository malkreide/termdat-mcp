...
- Dual-Transport: `stdio` (lokal) und Streamable HTTP (Cloud). Beide bedienen die MCP-Protokollrevision `2026-07-28`.
...
```bash
TERMDAT_MCP_TRANSPORT=streamable-http PORT=8000 termdat-mcp   # exponiert /mcp
```
...
├── src/termdat_mcp/
│   ├── __init__.py
│   ├── __main__.py       # Einstiegspunkt; Dual-Transport (stdio / Streamable HTTP)
...
- **Nutzungsbedingungen sind erfragt, nicht vermutet.** Jede Antwort führt sie im Feld `source`: Weiterverwendung und Weiterveröffentlichung sind nur mit Quellenangabe `www.termdat.ch` zulässig, und die Sektion Terminologie der Bundeskanzlei ist vorgängig über Zweck und Art zu informieren ([Auskunft vom 21.08.2026](docs/bk-auskunft-2026-08-21.md)).
...
- **Loopback als Default.** Der HTTP-Transport bindet an `127.0.0.1`; `0.0.0.0` ist ein expliziter Container-Opt-in mit stderr-Warnung. Er setzt zudem Default-Deny-CORS und exponiert nur `Mcp-Session-Id`.
...
- **Container.** Ein gehärtetes, non-root [`Dockerfile`](Dockerfile) liegt für gehostete Deployments bei.
...
- **Die öffentliche API liefert weniger als die Website — und zwar bewusst.** Keine Scope-Einstellung und kein Defekt: Auf Anfrage hat die Sektion Terminologie der Bundeskanzlei [am 21.08.2026 bestätigt](docs/bk-auskunft-2026-08-21.md), dass die API nur einen Teil der TERMDAT-Einträge abbildet, dass sich die Auswahl **an den Bedürfnissen der Übersetzerinnen und Übersetzer der Bundesverwaltung** orientiert und dass eine **vollständigere Abdeckung nicht geplant** ist. Vorher gemessen: Für «Quellensteuer» zeigt die Website 12 verschiedene Einträge, die API liefert bei maximalem Recall 7 (alle Sprachen, alle 11 Felder, Infix-Wildcard, alle Klassifikationen und Sammlungen); die Überschneidung beträgt **einen** Eintrag, 447912. Ein direkter Abruf der fehlenden IDs über `/v2/Entry` gibt HTTP 200 mit leerem Body zurück — sie werden gar nicht ausgeliefert, also erreicht sie keine Query. Folge: Fehlt ein Begriff in diesem Server, fehlt er in der **API** — nicht in TERMDAT; und es gibt nichts zu umgehen. Verifiziert am 30.07.2026 anhand der Entry-IDs, die @dfch in [Issue #11](https://github.com/malkreide/termdat-mcp/issues/11) geliefert hat.
- **Der I14Y-Katalogeintrag führt `license: null` — die Bedingungen stehen anderswo.** Weiterverwendung und Weiterveröffentlichung von TERMDAT-Inhalten sind nur mit Quellenangabe `www.termdat.ch` zulässig, und die Sektion Terminologie der Bundeskanzlei ist vorgängig über Zweck und Art zu informieren (`terminologie@bk.admin.ch`; [Auskunft vom 21.08.2026](docs/bk-auskunft-2026-08-21.md)). Der Betrieb dieses Servers ist durch die Anfrage abgedeckt, die zu dieser Auskunft führte; eine eigene Weiterveröffentlichung stromabwärts braucht eine eigene Meldung. Jede Antwort wiederholt die Bedingungen im Feld `source`.
...
Dieser Server bedient **zwei Protokoll-Aeren** ueber denselben Endpunkt, auf
beiden Transporten. Die erste Anfrage einer Verbindung entscheidet, welche gilt;
ein spaeterer Anspruch aus der jeweils anderen Aera wird abgewiesen.
...
**Dieser Pin allein hat einmal eine echte Luecke verdeckt.** Bis zum 18.09.2026
war der Netz-Transport die SSE-App des SDK, und die kennt gar keinen Zweig in
die moderne Aera — ueber HTTP war `2026-07-28` unerreichbar, waehrend die
Konstanten etwas anderes sagten und stdio sie bediente. Seit dem Wechsel auf
Streamable HTTP unter `/mcp` sind beide Aeren zusaetzlich *gemessen*:
[`tests/test_streamable_http.py`](tests/test_streamable_http.py) schickt echte
Anfragen beider Art durch den zusammengebauten ASGI-Stack und prueft die
ausgehandelte Revision, den Pflicht-`resultType` und die `-32022`-Abweisung
einer unbekannten modernen Revision.
...
MIT für diesen Server — siehe [LICENSE](LICENSE). Die TERMDAT-Inhalte unterliegen den Bedingungen der Bundeskanzlei: Quellenangabe `www.termdat.ch` und vorgängige Information der Sektion Terminologie über Zweck und Art jeder Weiterverwendung oder Weiterveröffentlichung ([Auskunft vom 21.08.2026](docs/bk-auskunft-2026-08-21.md)).
