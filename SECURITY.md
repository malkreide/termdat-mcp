...
| Binding | `stdio` by default (no network surface). The Streamable HTTP transport binds to `HOST`, default `127.0.0.1` (loopback); `0.0.0.0` is an explicit opt-in and warns on stderr outside a container (SEC-016) |
...
- **Network binding for hosted deployments** — the HTTP transport defaults to
